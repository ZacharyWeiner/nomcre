class PageSectionsController < ApplicationController
  ...
  def destroy
    page_id = @page_section.page.id
    @page_section.destroy

    respond_to do |format|
      format.html { redirect_to edit_page_path(page_id), notice: 'Page section was successfully destroyed.' }
  ...
  private
    ...
    def page_section_params
      params.require(:page_section).permit(:page_id, :title, :content, :order, :is_blog)
    end
end
